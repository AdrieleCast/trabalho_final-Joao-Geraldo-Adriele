	...
	funcao devolverKart(cadeia &locado[],inteiro &totalKarts,cadeia modelo[], inteiro diaAtualizado){
		...
		para(i=0; i<totalKarts; i++){
			...
			se(locado[i] == "Sim"){
				...
				escreva("O carro modelo: ", modelo[i], " está locado, tecle ", i, " para devolve-lo: \n")
				leia(verificaKart[i])

				
				se(verificaKart[i] == kartLocado[i]){
					
					locado[i] = "Não"
					escreva("O kart modelo ", modelo[i], " foi devolvido ! \n")
			
				}senao{
					escreva("Não existe kart com número ", verificaKart[i], " ainda \n")
				}
 ...
 * 
 * @POSICAO-CURSOR = 4804; 
 * @DOBRAMENTO-CODIGO = [4, 38, 63, 86, 118, 152];
 * @PONTOS-DE-PARADA = ;
